...
const HoaDonBH = (props) => {
    ...
    const fetchInvoice = useCallback(async (hoadonId) => {
        try {
            const data = await onGetInfoHDMTT(hoadonId, typeMaLoaiHoaDonMTT.BH)
    ...
    return (
        <SafeAreaView style={AppStyles.styleSafeArea}>
            ...
            <AppHeader
                ...
                title={I18n.t('menuLeftScreen.lapHoaDonBGMTT')}
                        ...
                        ListFooterComponent={() => {
                            return (
                                <View style={{ paddingHorizontal: AppSizes.paddingSml }}>
                                    <SummaryView data={Invoice} onPress={() => onAddHangHoaDV?.('BHMTT')} disabled={!isShowClose} />
                                </View>
                            )
                        }}
                    />           
