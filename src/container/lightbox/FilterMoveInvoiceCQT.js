...
class FilterSendInvoiceCQT extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      symbolSelected: mockAll,
      denominatorSelected: mockAll,
  ...
  async componentDidMount() {
    const {filterSelecled, dataLoaiHoaDon, dataKyHieu, dataMauSo} = this.props;
    if (dataLoaiHoaDon) {
      const mixDataLoaiHoaDon = [...dataLoaiHoaDon, mockAll];
      const mixDataKyHieu = [...dataKyHieu, mockAll];
      const mixDataMauSo = [...dataMauSo, mockAll];
      this.setState({
        dataLoaiHoaDon: mixDataLoaiHoaDon,
        dataKyHieu: mixDataKyHieu,
        dataMauSo: mixDataMauSo,
      });
    }
  ...
  static show(props) {
    Actions.filterMoveInvoiceCQT({...props});
  }
  ...
  renderSymbolItem = (item, index) => {
    ...
    const ischecked = symbolSelected.value == item.value;
    return (
      <TextWthRadio
        key={index.toString()}
        onPress={() => this.setState({symbolSelected: item, isSymbols: false})}
        ischecked={ischecked}
        title={item.text}
      />
    );
  ...
  renderDenominatorItem = (item, index) => {
    const {denominatorSelected} = this.state;
    const ischecked = denominatorSelected.value == item.value;
    return (
      <TextWthRadio
        key={index.toString()}
        onPress={() =>
          this.setState({denominatorSelected: item, isDenominator: false})
        }
        ischecked={ischecked}
        title={item.text}
      />
            ...
            <ScrollView style={{flex: 1, width: '100%', marginBottom: 70}}>
              ...
              <View style={{paddingHorizontal: AppSizes.paddingSml}}>
                <EnvoiceButtonTextDrop
                  ...
                  disabled
                  title={'Ký hiệu'}
                  value={symbolSelected.text}
                  onPress={() => {
                    this.setState({isSymbols: !isSymbols});
                  }}
                />
                {isSymbols &&
                  dataKyHieu.map((item, index) => {
                    return this.renderSymbolItem(item, index);
                  })}
              </View>
              ...
              <View style={{paddingHorizontal: AppSizes.paddingSml}}>
                <EnvoiceButtonTextDrop
                  ...
                  disabled
                  title={'Mẫu số'}
                  value={denominatorSelected.text}
                  onPress={() => {
                    this.setState({isDenominator: !isDenominator});
                  }}
                />
                {isDenominator &&
                  dataMauSo.map((item, index) => {
                    return this.renderDenominatorItem(item, index);
                  })}
              </View>
