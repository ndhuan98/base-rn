...
const KhachHangRow = ({ data }) => {
  return (
    <View style={{ width: '100%' }}>
      <Text style={{ ...AppStyles.boldText, fontSize: 12 }}>{data.ten_doi_tac}</Text>
      <Text style={AppStyles.hintText}>{`MKH: ${data.ma_doi_tac}`}</Text>
...
class CustomerList extends Component {
  ...
  onSelectedItem = item => {
    const mapItem = {
      ...item, ten_dv: item.ten_doi_tac
    }
    this.props.actionSelected && this.props.actionSelected(mapItem);
    this.refs.body.fadeOutRight(300).then(() => Actions.pop());
  };
  ...
  requestDataFormSever = () => {
    ...
    return API.Partner(params).then(
      res => {
