...
const useDonViTinh = () => {
  ...
  React.useEffect(() => {
    ...
    API.DanhSachHinhThucThanhToan(params).then(
      res => {
...
const Item = ({item, index, onRefresh}) => {
  const data = {
    id: item.maHinhthucttId,
    callbackref: () => onRefresh(),
  };
  return (
    <TouchableOpacity onPress={() => Actions.thongTinHinhThucThanhToan(data)}>
      <View style={styles.item}>
        <Text style={styles.id}>{index + 1}</Text>
        <View style={styles.content}>
          <Text numberOfLines={1} style={styles.title}>{item.tenHinhthuctt}</Text>
          <Text numberOfLines={1} style={styles.name}>{item.maHinhthuctt}</Text>
        </View>
      </View>
    ...
    <SafeAreaView style={AppStyles.styleSafeArea}>
      <View style={styles.container}>
        <AppHeader
          ...
          title={I18n.t('DanhSachHinhThucThanhToan.title')}
          subTitle={`(${totalCustomer} dữ liệu)`}
          RightIcon
          IconRight={PlusFull}
          onPressMenu={() => Actions.drawerOpen()}
          onPressRight={() =>
            Actions.themSuaHinhThucThanhToan({callbackref: () => onRefresh()})
          }
        />
        <View style={styles.warpSearch}>
          ...
          <TextInput
            ...
            placeholder={I18n.t('DanhSachHinhThucThanhToan.placeholder')}
