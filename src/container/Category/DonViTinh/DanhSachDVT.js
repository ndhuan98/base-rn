...
const useDonViTinh = () => {
  ...
  React.useEffect(() => {
    ...
    API.DanhSachDonViTinh(params).then(
      res => {
...
const Item = ({item, index, onRefresh}) => {
  ...
  return (
    <TouchableOpacity onPress={() => Actions.thongTinDonViTinh(data)}>
      <View style={styles.item}>
        ...
        <View style={styles.content}>
          ...
          <Text numberOfLines={1} style={styles.name}>{item.maDvt}</Text>
        </View>
      </View>
    ...
    <SafeAreaView style={AppStyles.styleSafeArea}>
      <View style={styles.container}>
        <AppHeader
          ...
          title={I18n.t('DanhSachDonViTinh.title')}
          subTitle={`(${totalCustomer} dữ liệu)`}
          RightIcon
          IconRight={PlusFull}
          onPressMenu={() => Actions.drawerOpen()}
          onPressRight={() =>
            Actions.themSuaDonViTinh({callbackref: () => onRefresh()})
          }
        />
        <View style={styles.warpSearch}>
          ...
          <TextInput
            ...
            placeholder={I18n.t('DanhSachDonViTinh.placeholder')}
