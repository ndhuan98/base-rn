...
const Item = props => {
    ...
    if (item.item.trangThaiTruyenNhan >= 9) {
        colorStatus = AppColors.customSuccess
    }
    if (item.item.trangThaiTruyenNhan < 0) {
    ...
    return (
        <TouchableOpacity onPress={() => onClickItem(item.item)}
            ...
            <View style={styles.horizontalItem}>
                <Text style={{ ...AppStyles.boldText }}>{item?.item?.soThongBao}</Text>
                <Text style={{ ...AppStyles.baseText }}>
                    Số lượng HĐ: <Text style={{ ...AppStyles.baseText, color: AppColors.black }}>{item?.item?.tongHdSs}</Text>
                </Text>
            </View>
            <View style={styles.horizontalItem}>
                <Text style={{ ...AppStyles.baseText, color: colorStatus }}>
                    {item?.item?.trangThaiTruyenNhanString}
                </Text>
                <Text style={{ ...AppStyles.baseText }}>{moment(item?.item?.ngayThongBao).format('DD/MM/YYYY')}</Text>
            </View>
        </TouchableOpacity>
    )
}
...
const useSwapItem = () => {
    const onClickItem = item => {
        return Actions.XuLyXoaBoB2({ formList: item, title: 'Thông báo sai sót 04/SS-HĐĐT' });
    };
    return { onClickItem };
};
...
const useSwapiInvoice = () => {
    ...
    React.useEffect(() => {
        ...
        const params = {
            ...
            endDate: endDate,
            pagecount: 20,
            tinhTrangHoaDon: statusSelected?.value || '',
            loaiThongBao: typeSelected?.value || '',
        };
        API.DanhSachThongBaoSaiSot(params).then(
            res => {
        ...
        <SafeAreaView style={AppStyles.styleSafeArea}>
            ...
            <View style={styles.container}>
                <AppHeader
                    ...
                    subTitle={`(${totalInvoice} Thông báo)`}
                    RightIcon
                    IconRight={Filter}
...
export default React.memo(ThongBaoSaiSot);
